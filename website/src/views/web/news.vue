  ...
  <div class="news">
    <div class="box">
      <div class="news-banner">
        ...
        <el-autocomplete   class="search-input" popper-class="my-autocomplete" highlight-first-item v-model="searchNews"
...
import { reqGetNewsList, reqGetRecomNews, reqSearchNewsList } from '@/api/new';
...
let newsItems = ref<Record<any, any>>({});
let newsTabs = ref<newsItems[]>([]);
let hotNews = ref<Array<any>>([]);
let pageInfo = ref<{
  activeName: string;
  pagenum: number;
  pagesize: number;
...
// 新闻搜索
let searchList = ref<Array<any>>([]);
// 卡片新闻
let recomNews = ref<Array<any>>([]);

async function querySearchAsync(queryString: string, cb: (list: Array<any>) => void): Promise<any> {
  const { data: res } = await reqSearchNewsList(queryString);
 ...
 * @param event
 */
function handleClick(tab: TabsPaneContext, event: Event): any {
  getNewsItems();
}

// 根据新闻 类型、日期 查询新闻，并按日期排序
async function getNewsItems() {
  const { data: res } = await reqGetNewsList(pageInfo.value);
  if (res.status !== 200) {
    newsItems.value = {};
  } else {
...
async function getRecomNews() {
  const { data: res } = await reqGetRecomNews();
  if (res.status !== 200) {
    hotNews.value = [];
  } else {
